...
export default function GitRecipesDirectoryPage() {
  const allRecipes = useMemo(() => getAllGitRecipes(), []);
    ...
    <div className="space-y-8 py-2">
      ...
      <div className="space-y-3">
        <nav aria-label="Breadcrumb" className="flex items-center space-x-2 text-xs font-mono text-zinc-400">
          ...
          <span className="text-brand-emerald font-semibold">Git Command Recipes</span>
        </nav>
      ...
      <div className="space-y-4">
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search git commands (e.g. undo commit, discard changes, stash, cherry-pick, conflict)..."
        ...
        {filteredRecipes.map((recipe) => (
          ...
          >
            <div className="space-y-2">
              ...
              <Link href={`/git/${recipe.slug}/`} className="block">
      ...
      {filteredRecipes.length === 0 && (
        <div className="p-12 text-center rounded-2xl bg-surface-100 border border-border text-zinc-400 space-y-2">
          <Terminal className="w-8 h-8 text-zinc-500 mx-auto" />
          <p className="text-sm">No Git recipes found matching &quot;{searchTerm}&quot;</p>
        </div>
      )}
    </div>
  );
