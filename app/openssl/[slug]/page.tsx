...
import {
  getAllOpenSslRecipes,
  getOpenSslRecipeBySlug,
  OPENSSL_RECIPES,
} from '@/lib/openssl-recipes-data';
import { OpenSslCommandBuilder } from '@/components/openssl/OpenSslCommandBuilder';
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const recipe = getOpenSslRecipeBySlug(params.slug);
  if (!recipe) return { title: 'OpenSSL Command Recipe' };

  const title = `${recipe.title} | DevTransform`;
  const description = `${recipe.summary} Includes copyable OpenSSL CLI commands, interactive parameter customization, flag breakdowns, and verification steps.`;
  const canonicalUrl = `https://devtransform-hub.vercel.app/openssl/${recipe.slug}/`;
  ...
  return {
    ...
    keywords: [
      ...
      `openssl ${recipe.shortTitle.toLowerCase()}`,
      'openssl commands',
      'ssl certificate tutorial',
      'x509 certificate generation',
      'pem crt key conversion',
    ],
    alternates: {
...
export default function OpenSslRecipePage({ params }: PageProps) {
  const recipe = getOpenSslRecipeBySlug(params.slug);
  if (!recipe) notFound();

  const relatedRecipes = OPENSSL_RECIPES.filter((r) => r.slug !== recipe.slug).slice(0, 6);
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 py-12 px-4 sm:px-6 lg:px-8">
      ...
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        <nav className="flex items-center gap-2 text-xs text-slate-400">
          ...
          <Link href="/openssl/directory/" className="hover:text-white transition">
            OpenSSL Directory
          </Link>
          <ChevronRight className="w-3 h-3" />
          <span className="text-emerald-400 font-medium">{recipe.shortTitle}</span>
        </nav>
        ...
        <div className="space-y-4">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
            ...
            <span>OpenSSL &bull; {recipe.category}</span>
          </div>
        ...
        <OpenSslCommandBuilder recipe={recipe} />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
            {relatedRecipes.map((r) => (
              <Link
                key={r.slug}
                href={`/openssl/${r.slug}/`}
                className="p-4 rounded-xl bg-slate-900/60 border border-slate-800 hover:border-emerald-500/50 hover:bg-slate-800/50 transition group flex flex-col justify-between"
              >
                <div>
