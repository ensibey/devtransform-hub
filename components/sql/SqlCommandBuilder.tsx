...
import { SqlRecipe } from '@/lib/sql-recipes-data';
...
interface SqlCommandBuilderProps {
  recipe: SqlRecipe;
}

export function SqlCommandBuilder({ recipe }: SqlCommandBuilderProps) {
  const getRiskBadge = () => {
      ...
      case 'Destructive':
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-mono font-semibold bg-rose-950/40 text-rose-400 border border-rose-500/30">
            ...
            Destructive (Table / Data Deletion)
          </span>
        );
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="p-5 rounded-2xl bg-surface-100 border border-border shadow-sm space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center space-x-2">
            {getRiskBadge()}
            <span className="px-2 py-0.5 rounded text-[11px] font-mono bg-surface-200 text-brand-emerald border border-border">
              {recipe.dialect}
            </span>
        ...
        <div className="p-3.5 rounded-xl bg-black/70 border border-border flex items-start justify-between gap-3">
          <pre className="text-xs sm:text-sm font-mono text-emerald-400 overflow-x-auto whitespace-pre">
            <code>{recipe.quickQuery}</code>
          </pre>
      ...
      <div className="space-y-4">
        <h2 className="text-lg font-bold text-white flex items-center gap-2">
          ...
          <span>Step-by-Step Query Breakdown</span>
        </h2>
